import React, { useState, useEffect } from 'react';

export const CRTOverlay: React.FC = () => {
  const [enabled, setEnabled] = useState<boolean>(true);
  const [lowPerformance, setLowPerformance] = useState<boolean>(false);

  useEffect(() => {
    try {
      const saved = localStorage.getItem('hypercade_crt');
      if (saved !== null) setEnabled(saved === 'true');
    } catch {}

    // Check device capability or user motion preference
    try {
      const isMobile = window.innerWidth < 768 || ('ontouchstart' in window && window.innerWidth < 1024);
      const isLowCores = typeof navigator !== 'undefined' && navigator.hardwareConcurrency && navigator.hardwareConcurrency <= 4;
      const prefersReducedMotion = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
      if (isMobile || isLowCores || prefersReducedMotion) {
        setLowPerformance(true);
      }
    } catch {}
  }, []);

  if (!enabled) return null;

  return (
    <div 
      className="pointer-events-none fixed inset-0 z-50 overflow-hidden select-none"
      style={{
        willChange: 'transform',
        transform: 'translateZ(0)',
      }}
    >
      {/* Scanline pattern with GPU composited layer */}
      <div 
        className="absolute inset-0 opacity-[0.14] mix-blend-overlay"
        style={{
          backgroundImage: 'repeating-linear-gradient(0deg, rgba(0, 0, 0, 0.9) 0px, rgba(0, 0, 0, 0.9) 1px, transparent 1px, transparent 3px)',
          backgroundSize: '100% 3px',
          willChange: 'transform',
          transform: 'translateZ(0)',
        }}
      />
      {/* Subtle arcade vignette */}
      <div className="absolute inset-0 bg-radial from-transparent via-transparent to-black/60 pointer-events-none" />
      
      {/* Moving faint scanline sweep - automatically disabled on low-performance screens to eliminate animation reflow overhead */}
      {!lowPerformance && (
        <div 
          className="absolute inset-x-0 h-24 bg-gradient-to-b from-transparent via-white/[0.02] to-transparent animate-scanline"
          style={{
            willChange: 'transform',
            transform: 'translateZ(0)',
          }}
        />
      )}
    </div>
  );
};
