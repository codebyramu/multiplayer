...
export class BotAIController {
  public static computeInput(
    ...
    const isPerceptionTick = (state.tickCount % 5 === 0) || state.cachedDesiredAngle === undefined;
    ...
    if (isPerceptionTick) {
      ...
      state.cachedAction1 = wantsJump;
      state.cachedAction2 = action2;
    }

    // 5. Compute Steer Angle & Analog Vectors with smooth lerp
    const desiredAngle = state.cachedDesiredAngle ?? bot.angle;
    let angleDelta = desiredAngle - bot.angle;
    while (angleDelta > Math.PI) angleDelta -= Math.PI * 2;
    while (angleDelta < -Math.PI) angleDelta += Math.PI * 2;
    ...
    return {
      ...
      angle: inputAngle,
      magnitude: 1.0,
      action1: !!state.cachedAction1,
      action2: !!state.cachedAction2,
      timestamp: Date.now(),
    };
  }
}
