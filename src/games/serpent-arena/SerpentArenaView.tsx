...
import { Volume2, VolumeX, Pause, Play, RotateCcw, Trophy, Skull } from 'lucide-react';

import { SerpentModifierId, SerpentModifiersConfig } from './types';

interface SerpentArenaViewProps {
...
export const SerpentArenaView: React.FC<SerpentArenaViewProps> = ({
  players,
  ...
  const engineRef = useRef<SerpentArenaEngine | null>(null);
  ...
  // Initialize Serpent Arena Engine
  useEffect(() => {
    const engine = new SerpentArenaEngine({
      roundDuration,
      modifiers,
    });

    // Populate all participating players (humans & bots)
    for (const pid in players) {
      engine.addPlayer(players[pid]);
    }
  ...
  }, [players, roundDuration, modifiers, onBroadcastEvent]);

  // Sync keyboard inputs into direction angle and boost
  const updateLocalInputFromKeys = useCallback(() => {
    ...
    const isBoosting = keys.has('Space') || keys.has('ShiftLeft') || keys.has('ShiftRight');
    ...
    if (dx !== 0 || dy !== 0) {
      ...
      localInputRef.current.action1 = isBoosting;
    }
  }, []);
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (dist > 15) {
      ...
      localInputRef.current.magnitude = Math.min(1.0, dist / 200);
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (!isPaused && engineRef.current) {
        ...
        // 4. Check Match Over (Last-Man-Standing or Timer Expiration)
        const isFinished = engine.isMatchOver;
  ...
  }, [isPaused, clientInputs, localPlayerId, onBroadcastState, onMatchEnd, roundDuration]);
  ...
  const handleRestart = () => {
    ...
    const engine = new SerpentArenaEngine({ roundDuration, modifiers });
    for (const pid in players) {
      engine.addPlayer(players[pid]);
    }
    engine.onEvent((event) => {
  ...
  return (
    <div className="relative w-full h-full min-h-[600px] bg-arcade-obsidian overflow-hidden select-none">
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 pointer-events-none">
        <div className="px-4 py-2 rounded-full bg-arcade-dark/85 backdrop-blur border border-arcade-mint/30 text-xs font-mono text-white/90 flex items-center gap-3 shadow-lg shadow-arcade-mint/10">
          <span className="text-arcade-mint font-bold">STEER:</span> Mouse Cursor / WASD / Arrow Keys
          <span className="text-white/30">|</span>
          <span className="text-arcade-cyan font-bold">HYPER BOOST:</span> Left Click / Spacebar
        </div>
      </div>
      ...
      {isPaused && (
        <div className="absolute inset-0 z-40 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <GlassPanel className="p-8 max-w-sm w-full text-center border-arcade-cyan/40">
            <h2 className="text-2xl font-black font-arcade text-arcade-cyan mb-2">GAME PAUSED</h2>
            <p className="text-sm font-mono text-white/60 mb-6">Serpent arena simulation suspended.</p>
            <div className="flex flex-col gap-3">
              <ArcadeButton variant="mint" onClick={() => setIsPaused(false)}>
                RESUME BATTLE
              </ArcadeButton>
      ...
      {matchOverResults && (
        <div className="absolute inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4">
          <GlassPanel className="p-8 max-w-lg w-full text-center border-arcade-mint/50 animate-in fade-in zoom-in duration-300">
            <div className="inline-flex p-3 rounded-full bg-arcade-mint/20 text-arcade-mint mb-4 ring-2 ring-arcade-mint/40">
            ...
            <p className="text-xl font-bold font-mono text-arcade-mint mb-6">
            ...
            <div className="space-y-2 mb-8 max-h-48 overflow-y-auto pr-1">
              {matchOverResults.rankings.map((r, i) => (
                <div
                  key={r.id}
            ...
            <div className="flex items-center justify-center gap-4">
              <ArcadeButton variant="mint" onClick={handleRestart} icon={<RotateCcw className="w-4 h-4" />}>
                PLAY AGAIN
              </ArcadeButton>
            </div>
          </GlassPanel>
